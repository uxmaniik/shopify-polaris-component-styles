...
Element: `s-grid`  
Docs: https://shopify.dev/docs/api/app-home/web-components/layout-and-structure/grid  
Captured: 2026-05-03  
...
Use the native `s-grid` component before creating custom Shopify-admin-looking UI.

## Native Examples

```tsx
<s-grid data-measure="target" ><s-text>Example content</s-text><s-button>Action</s-button></s-grid>
```
...
| --- | --- | --- | --- | --- |
| `gridTemplateColumns` | string | 'none' | yes | The columns in the grid and their sizes. Accepts: [Track sizing values](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_grid_layout/Basic_concepts_of_grid_layout#fixed_and_flexible_track_sizes), such as `1fr auto... |
| `gridTemplateRows` | string | 'none' | yes | The rows in the grid and their sizes. Accepts: [Track sizing values](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_grid_layout/Basic_concepts_of_grid_layout#fixed_and_flexible_track_sizes), such as `1fr auto` A... |
| `justifyItems` | "" \| JustifyItemsKeyword | '' - meaning no override | yes | Aligns the grid items along the inline axis. |
| `alignItems` | "" \| AlignItemsKeyword | '' - meaning no override | yes | Aligns the grid items along the block axis. |
| `placeItems` | 197 allowed string values; see Shopify docs for the exhaustive union. | 'normal normal' | yes | A shorthand property for `justify-items` and `align-items`. |
| `justifyContent` | "" \| JustifyContentKeyword | '' - meaning no override | yes | Aligns the grid along the inline axis. This overrides the inline value of `placeContent`. |
| `alignContent` | "" \| AlignContentKeyword | '' - meaning no override | yes | Aligns the grid along the block axis. This overrides the block value of `placeContent`. |
| `placeContent` | 239 allowed string values; see Shopify docs for the exhaustive union. | 'normal normal' | yes | A shorthand property for `justify-content` and `align-content`. |
| `gap` | MaybeResponsive\<MaybeTwoValuesShorthandProperty\<SpacingKeyword>> | 'none' | yes | Adjusts spacing between elements. Accepts: A single [`SpacingKeyword`](https://shopify.dev/docs/api/polaris/using-polaris-web-components#scale) value applied to both axes, such as `large-100` A pair of values, such as... |
| `rowGap` | MaybeResponsive<"" \| SpacingKeyword> | '' - meaning no override | yes | s spacing between elements in the block axis. This overrides the row value of `gap`. Accepts: A single [`SpacingKeyword`](https://shopify.dev/docs/api/polaris/using-polaris-web-components#scale) value, such as `large-... |
| `columnGap` | MaybeResponsive<"" \| SpacingKeyword> | '' - meaning no override | yes | Adjusts spacing between elements in the inline axis. This overrides the column value of `gap`. Accepts: A single [`SpacingKeyword`](https://shopify.dev/docs/api/polaris/using-polaris-web-components#scale) value, such ... |
...
| Slot | Type | Notes |
| --- | --- | --- |
| `children` | HTMLElement | The content displayed within the grid item component, which represents a single cell in the grid layout and can span multiple columns or rows. ** |
...
Notes: Open shadow root inspected. Measured internal span node.

Host rect: `{"x":0,"y":0,"width":0,"height":0}`  
First visible shadow/control rect: `{"x":32,"y":32,"width":960,"height":48}`
...
- Keep new wrappers thin and delegate visual behavior to `s-grid`.
