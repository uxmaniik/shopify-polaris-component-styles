...
Element: `s-money-field`  
Docs: https://shopify.dev/docs/api/app-home/web-components/forms/money-field  
Captured: 2026-05-03  
...
Use the native `s-money-field` component before creating custom Shopify-admin-looking UI.

## Native Examples

```tsx
<s-money-field data-measure="target" label="Money field">Money field</s-money-field>
```
...
| --- | --- | --- | --- | --- |
| `max` | number | Infinity | yes | The highest decimal or integer value accepted for the field. When used with `step`, the value rounds down to the maximum number. Users can still type values higher than the maximum using the keyboard. Implement valida... |
| `min` | number | -Infinity | yes | The lowest decimal or integer value accepted for the field. When used with `step`, the value rounds up to the minimum number. Users can still type values lower than the minimum using the keyboard. Implement validation... |
| `value` | string | none listed | yes | The current monetary value in the field as a string. When setting this property programmatically, it updates the field's display value. When reading it, you get the user's current input. The value should be a numeric ... |
| `autocomplete` | string | 'on' for everything else | yes | Controls browser autofill behavior for the field. Basic values: `on` - Enables autofill without specifying content type (default) `off` - Disables autofill for sensitive data or one-time codes Specific field values de... |
| `defaultValue` | string | none listed | yes | The initial value of the field when it first loads. Unlike `placeholder`, this is a real value that the user can edit and that gets submitted with the form. Once the user starts typing, their input replaces it. Changi... |
...
| Event | Type | Notes |
| --- | --- | --- |
| `blur` | CallbackEventListener<'input'> | A callback fired when the money field loses focus. Learn more about the [blur event](https://developer.mozilla.org/en-US/docs/Web/API/Element/blur_event). |
| `change` | CallbackEventListener<'input'> | A callback fired when the money field value changes. Learn more about the [change event](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/change_event). |
| `focus` | CallbackEventListener<'input'> | A callback fired when the money field receives focus. Learn more about the [focus event](https://developer.mozilla.org/en-US/docs/Web/API/Element/focus_event). |
| `input` | CallbackEventListener<'input'> | A callback fired when the user inputs data into the money field. Learn more about the [input event](https://developer.mozilla.org/en-US/docs/Web/API/Element/input_event). |
...
First visible shadow/control rect: `{"x":44,"y":62,"width":936,"height":20}`
...
- Keep new wrappers thin and delegate visual behavior to `s-money-field`.
