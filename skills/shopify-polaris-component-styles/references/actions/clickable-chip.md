...
Element: `s-clickable-chip`  
Docs: https://shopify.dev/docs/api/app-home/web-components/actions/clickable-chip  
Captured: 2026-05-03  
...
Use the native `s-clickable-chip` component before creating custom Shopify-admin-looking UI.

## Native Examples

```tsx
<s-clickable-chip data-measure="target" >Clickable chip</s-clickable-chip>
```
...
| --- | --- | --- | --- | --- |
| `color` | ColorKeyword | 'base' | yes | The color emphasis level that controls visual intensity. |
...
| `removable` | boolean | false | yes | Whether the chip displays a remove button for dismissal. When clicked, the `remove` callback fires. |
| `hidden` | boolean | false | yes | Whether the chip is hidden from view. When using controlled component pattern with `removable` chips, update this property when the `remove` event fires. For non-removable chips, manually toggle this property to show ... |
| `disabled` | boolean | false | yes | Whether the chip is disabled, preventing any user interaction. |
...
| Slot | Type | Notes |
| --- | --- | --- |
| `children` | HTMLElement | The text label displayed within the chip, which represents an interactive filter, tag, or selectable item. |
| `graphic` | HTMLElement | An optional icon to display at the start of the chip. Accepts only icon components. ** |

## Events

| Event | Type | Notes |
| --- | --- | --- |
| `afterhide` | CallbackEventListener\<typeof tagName> \| null | A callback fired after the chip is hidden. |
| `click` | CallbackEventListener\<typeof tagName> \| null | A callback fired when the chip is clicked. Learn more about the [click event](https://developer.mozilla.org/en-US/docs/Web/API/Element/click_event). |
| `remove` | CallbackEventListener\<typeof tagName> \| null | A callback fired when the chip is removed. |
...
First visible shadow/control rect: `{"x":40,"y":36,"width":81.06,"height":16}`
...
Host rect: `{"x":0,"y":0,"width":0,"height":0}`  
First visible shadow/control rect: `{"x":40,"y":36,"width":80.08,"height":16}`
...
- Keep new wrappers thin and delegate visual behavior to `s-clickable-chip`.
