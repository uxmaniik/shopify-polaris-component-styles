...
Element: `s-icon`  
Docs: https://shopify.dev/docs/api/app-home/web-components/media-and-visuals/icon  
Captured: 2026-05-03  
...
Use the native `s-icon` component before creating custom Shopify-admin-looking UI.

## Native Examples

```tsx
<s-icon data-measure="target" >Icon</s-icon>
```
...
| `type` | 559 allowed string values; see Shopify docs for the exhaustive union. | none listed | yes | The icon to display from the icon library. Set to a valid icon name to display that icon. To hide the icon completely, use an empty string `''`. To reserve the icon's space without displaying an icon, use `'empty'`. |
| `size` | "small" \| "base" | 'base' | yes | The size of the icon. `small`: Smaller icon suitable for inline use within text or compact UI elements. `base`: Default size that works well for standalone icons and standard use cases. |
...
Host rect: `{"x":32,"y":32,"width":960,"height":20}`  
First visible shadow/control rect: `{"x":32,"y":32,"width":20,"height":20}`
...
| border-bottom-style | none |
| border-left-style | none |
| border-top-color | rgb(226, 44, 56) |
| border-right-color | rgb(226, 44, 56) |
| border-bottom-color | rgb(226, 44, 56) |
| border-left-color | rgb(226, 44, 56) |
| border-radius | 0px |
| border-start-start-radius | 0px |
| border-start-end-radius | 0px |
| border-end-start-radius | 0px |
...
- Keep new wrappers thin and delegate visual behavior to `s-icon`.
